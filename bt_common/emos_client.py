...
from bt_common.exceptions import EMOSConnectionError, EMOSNotFoundError, EMOSError, EMOSValidationError
...
class EMOSClient:
    def __init__(
        ...
        *,
        ...
        try:
            ...
            raise EMOSError("evermemos SDK is required. Install with `pip install evermemos`.") from exc
    ...
    async def save_conversation_meta(self, payload: dict[str, Any]) -> dict[str, Any]:
        created_at = payload.get("created_at") or datetime.now(tz=timezone.utc).isoformat()
