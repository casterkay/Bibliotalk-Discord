...
from .errors import ConfigError, IngestError, InvalidInputError
from .evermemos_client import EverMemOSClient
from .index import IngestionIndex
from .ingest import ingest_manifest, ingest_sources
...
def _report_path(cli: CLIContext, *, run_id: str) -> Path:
    ...
    return (Path.cwd() / ".evermemos_ingest" / "reports" / f"{run_id}.json").resolve()
...
def ingest_text(
    ...
    try:
        cfg = load_runtime_config(emos_base_url=cli.emos_base_url, emos_api_key=cli.emos_api_key, index_path=cli.index_path)
        index = IngestionIndex(cfg.index_path)
        client = EverMemOSClient(cfg.emos_base_url, api_key=cfg.emos_api_key, timeout=cfg.emos_timeout_s, retries=cfg.emos_retries)
        ...
        report = _run(ingest_sources(sources=[source_content], index=index, client=client, redact_secrets=[cfg.emos_api_key or ""]))
        _run(client.aclose())
    except (ConfigError, InvalidInputError) as exc:
        raise typer.Exit(code=2) from exc
    except IngestError as exc:
        console.print(f"[red]Error:[/red] {redact_text(str(exc), secrets=[cli.emos_api_key or ''])}")
        raise typer.Exit(code=1) from exc

    write_report(report, path=_report_path(cli, run_id=report.run_id), secrets=[cfg.emos_api_key or ""])

    src = report.sources[0]
    logging.getLogger("evermemos_ingest").info("run_id=%s status=%s group_id=%s", report.run_id, report.status, src.group_id)
    console.print(f"run_id={report.run_id} group_id={src.group_id} status={src.status} ingested={src.segments_ingested} skipped={src.segments_skipped_unchanged} failed={src.segments_failed}")
...
def ingest_manifest(
    ...
    try:
        ...
        index = IngestionIndex(cfg.index_path)
        client = EverMemOSClient(cfg.emos_base_url, api_key=cfg.emos_api_key, timeout=cfg.emos_timeout_s, retries=cfg.emos_retries)
        report = _run(ingest_manifest(manifest=manifest, index=index, client=client, redact_secrets=[cfg.emos_api_key or ""]))
        _run(client.aclose())
    except (ConfigError, InvalidInputError) as exc:
    ...
    logging.getLogger("evermemos_ingest").info("run_id=%s status=%s sources_total=%s sources_failed=%s", report.run_id, report.status, report.summary.sources_total, report.summary.sources_failed)
    console.print(
