...
from .errors import IngestError, InvalidInputError
...
from .reporting import redact_text

logger = logging.getLogger("evermemos_ingest")
...
def _failed_source_result(*, source: Any, err: IngestError, redact_secrets: list[str], include_segment_details: bool) -> SourceResult:
    return SourceResult(
...
async def _source_content_from_resolved(resolved: ResolvedManifestSource) -> SourceContent:
    ...
    if resolved.mode == "file":
        ...
        return load_file_source(
            ...
            published_at=resolved.source.published_at.isoformat() if resolved.source.published_at else None,
        )
    ...
    if resolved.mode == "gutenberg":
        ...
        if resolved.gutenberg_id is None:
            raise InvalidInputError("Manifest source mode=gutenberg missing gutenberg_id")
    ...
    *,
    ...
    try:
        if not index.get_source_meta_saved(user_id=source.user_id, group_id=group_id):
            ...
            await client.save_conversation_meta(group_id=group_id, source_meta={k: v for k, v in meta.items() if v is not None})
            index.set_source_meta_saved(user_id=source.user_id, group_id=group_id, source_fingerprint=_source_fingerprint(source_content))
        meta_saved = True
    ...
    for seg in segments:
        ...
        if existing and existing.sha256 == seg.sha256 and existing.status in {"ingested", "skipped_unchanged"}:
            skipped += 1
