...
from bt_agent.voice.backends.base import EndOfTurn, Transcript
from bt_agent.voice.session_manager import VoiceSession
...
class TranscriptCollector:
    async def collect_and_post(self, session: VoiceSession, *, post_message: PostMessage, save_history: SaveHistory) -> None:
        buffer: list[str] = []
        async for event in session.backend.receive():
            ...
            if isinstance(event, EndOfTurn):
                ...
                payload = format_clone_response(text, [])
