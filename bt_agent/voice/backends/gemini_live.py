"""Gemini Live backend adapter (mockable local implementation)."""
...
from bt_agent.voice.backends.base import AudioChunk, EndOfTurn, Transcript, VoiceBackend, VoiceEvent
...
class GeminiLiveBackend(VoiceBackend):
    ...
    async def send_audio_chunk(self, pcm_16khz_bytes: bytes) -> None:
        if not self._active:
            return
        await self._queue.put(Transcript(text=f"gemini heard {len(pcm_16khz_bytes)} bytes", role="user"))
