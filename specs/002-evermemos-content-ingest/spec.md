...
### User Story 1 - Ingest A Single Source For A Ghost (Priority: P1)

As a curator or operator, I can ingest a single content source (for example: a book, an essay, or a time-coded transcript) into EverMemOS for a specific person/figure, so that the Ghost can later answer questions grounded in that material with verifiable citations.
...
- **SC-003**: In a validation set of 100 cited claims generated by Ghosts from ingested material, at least 90% of citations resolve to a verifiable verbatim segment with correct source attribution (title + canonical link + location when available).
...
This feature covers ingesting curated, consented content into EverMemOS to support grounded Ghost conversations with citations. It focuses on transforming operator-provided content into well-attributed, deduplicated, citation-friendly stored memory.
