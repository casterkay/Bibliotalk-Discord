...
- [X] T003 Update packaging config to include `ingestion_service` in `pyproject.toml`
...
- [X] T006 [P] Add a minimal package README for operators in `services/ingestion_service/src/README.md`
...
- [X] T008 Create typed exceptions for ingest failures in `services/ingestion_service/src/errors.py`
- [X] T009 Implement config loading from env + CLI overrides in `services/ingestion_service/src/config.py`
...
- [X] T019 [US1] Implement CLI root and `ingest` command group in `services/ingestion_service/src/cli.py`
- [X] T020 [US1] Implement `ingest text` command (args per `contracts/cli.md`) in `services/ingestion_service/src/cli.py`
- [X] T021 [US1] Implement `ingest file` command (args per `contracts/cli.md`) in `services/ingestion_service/src/cli.py`
- [X] T022 [US1] Wire CLI → adapter → chunking → ingestion pipeline in `services/ingestion_service/src/cli.py`
- [X] T023 [US1] Implement report file emission + exit codes for single-source runs in `services/ingestion_service/src/cli.py`
...
- [X] T024 [US2] Extend ingestion index to record per-segment sha/status and detect unchanged segments in `services/ingestion_service/src/index.py`
- [X] T025 [US2] Update ingestion pipeline to skip unchanged segments and increment `segments_skipped_unchanged` in `services/ingestion_service/src/ingest.py`
- [X] T026 [US2] Implement “changed content” behavior for same `message_id` (re-memorize + update index) in `services/ingestion_service/src/ingest.py`
- [X] T027 [US2] Add CLI options `--index-path` and default index path behavior in `services/ingestion_service/src/config.py`
- [X] T028 [US2] Ensure report output includes skip/update counts per `contracts/report-format.md` in `services/ingestion_service/src/reporting.py`
...
- [X] T029 [P] [US3] Define manifest schema models + validation per `contracts/ingest-manifest.md` in `services/ingestion_service/src/manifest.py`
- [X] T030 [US3] Implement `ingest manifest` command in `services/ingestion_service/src/cli.py`
- [X] T031 [US3] Implement adapter selection + allowlist for manifest sources in `services/ingestion_service/src/ingest.py`
...
- [X] T034 [US3] Implement batch runner (continue-on-error) + consolidated report assembly in `services/ingestion_service/src/ingest.py`
- [X] T035 [US3] Ensure unsupported/interactive-required sources fail with actionable errors in `services/ingestion_service/src/manifest.py`
- [X] T036 [US3] Ensure CLI exits with code `1` when any source fails and still writes report in `services/ingestion_service/src/cli.py`
...
- [X] T038 Add structured logging fields (`run_id`, `group_id`, `message_id`) in `services/ingestion_service/src/cli.py`
- [X] T039 Harden redaction: scrub secrets from exception messages and reports in `services/ingestion_service/src/reporting.py`
- [X] T040 Add consistent error codes for common failures (auth, network, invalid input, unsupported source) in `services/ingestion_service/src/errors.py`
...
```bash
Task: "Extend SQLite index in services/ingestion_service/src/index.py"
Task: "Update pipeline skip logic in services/ingestion_service/src/ingest.py"
```
