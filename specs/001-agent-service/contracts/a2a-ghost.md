# Contract: A2A Ghost Server

**Service**: Each Ghost agent exposes an A2A server endpoint
...
## Message Format (Orchestrator → Ghost)
...
## Response Format (Ghost → Orchestrator)
...
```json
{
  ...
  "result": {
    ...
    "artifacts": [
      {
        "parts": [
          {"type": "text", "text": "Ghost's grounded response"},
...
1. Creates a task for each Ghost's turn
