...
### User Story 3 — Grounded DM Chat With a Figure Bot (Priority: P3)

A Discord user sends a direct message to a figure bot. The bot retrieves relevant evidence from EverMemOS, reranks verbatim segments locally, generates a response that answers only from evidence using inline markdown memory links, and validates every link and quoted span before sending. If no usable evidence exists the bot explicitly says so.

**Why this priority**: The DM experience is the highest-value user-facing capability and the proof of the "言必有據" (evidence-backed speech) principle.

**Independent Test**: DM a figure bot with a question known to match an already-ingested transcript segment. Confirm the response contains an inline markdown link in the form `[text](memory_url)`, the linked page shows exactly one EMOS memory item with the video timepoint, and the generated quote is a substring of the cached verbatim segment text.

**Acceptance Scenarios**:

1. **Given** a DM question with a known matching transcript segment, **When** the bot responds, **Then** the response contains at least one inline markdown link of the form `[text](https://www.bibliotalk.space/memory/{user_id}_{timestamp})`.
2. **Given** a response containing a memory link, **When** the linked page is loaded, **Then** it shows exactly one EMOS memory item and a link to the original video at the reconstructed transcript timepoint.
3. **Given** a question with no supporting evidence in EverMemOS, **When** the bot responds, **Then** the bot explicitly states it lacks relevant evidence rather than fabricating an answer.
4. **Given** a generated response contains a memory link that does not resolve to a valid EMOS memory, **When** the citation validator runs, **Then** the invalid link is stripped before the message is sent to the user.
5. **Given** a generated quoted span is not a substring of the cached local segment text, **When** the citation validator runs, **Then** the bad quote is stripped or the link is removed before sending.
6. **Given** a DM directed to Figure A's bot, **When** the bot retrieves memories, **Then** only memories belonging to figure A are returned; no cross-figure data leaks.
...
- **FR-022**: When a user DMs a figure bot, the system MUST search EverMemOS for relevant evidence before generating any response.
- **FR-023**: The system MUST rerank retrieved EverMemOS candidate segments using BM25 against the user query.
- **FR-024**: Every non-trivial factual claim in a bot response MUST be expressed as an inline markdown link of the form `[text](https://www.bibliotalk.space/memory/{user_id}_{timestamp})`.
- **FR-025**: DM responses MUST NOT use citation indices (e.g., [1], [2]) or a trailing `Sources:` section.
- **FR-026**: The system MUST validate each `memory_url` in a generated response resolves to a real EMOS memory belonging to the current figure before sending.
- **FR-027**: The system MUST validate that any quoted span in the response is a substring of the corresponding cached local segment text before sending.
- **FR-028**: If no usable evidence remains after validation, the bot MUST respond that it could not find relevant supporting evidence.
- **FR-029**: Memory retrieval MUST be strictly scoped to the queried figure; cross-figure memory access is forbidden.

#### Memory Pages

- **FR-030**: The system MUST serve public memory pages at `https://www.bibliotalk.space/memory/{user_id}_{timestamp}` that contain exactly one EMOS memory item and a link to the original video at the reconstructed transcript timepoint.

#### Operations

- **FR-031**: The system MUST run as one OS process per figure bot (process-per-bot model).
- **FR-032**: Global and per-source ingest concurrency MUST be configurable and bounded, where a source is a single channel or playlist subscription.
- **FR-033**: Required secrets (`EMOS_BASE_URL`, `EMOS_API_KEY`, per-figure `DISCORD_TOKEN`) MUST be injected through environment variables.
...
- **SC-004**: At least 95% of DM responses to questions with known matching transcripts include a valid inline memory link that resolves to the correct EMOS memory page.
- **SC-005**: No DM response ever includes a memory link to a memory belonging to a different figure.
...
- One Discord bot per figure (process-per-bot).
- One feed channel and feed thread UX per figure.
- DM chat grounded on EverMemOS + local verbatim cache.
...
- Discord token provisioning (one per figure) is done by the operator out-of-band.
